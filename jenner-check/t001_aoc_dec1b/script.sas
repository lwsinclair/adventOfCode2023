data dec1b;
 length row $200 value first last numb total 8 str1 $1;
 retain total;
 ...
 do i = 1 to length(row);
	...
	/* numeric? */
	if anydigit(str1) then do;
		value = input(str1,best1.);
	end;

	/* not numeric, is it a string representing a number? */
	else do;
		if substr(row,i,3) = 'one' then value = 1;
		if substr(row,i,3) = 'two' then value = 2;
		if substr(row,i,5) = 'three' then value = 3;
		if substr(row,i,4) = 'four' then value = 4;
		if substr(row,i,4) = 'five' then value = 5;
		if substr(row,i,3) = 'six' then value = 6;
		if substr(row,i,5) = 'seven' then value = 7;
		if substr(row,i,5) = 'eight' then value = 8;
		if substr(row,i,4) = 'nine' then value = 9;
	end;

	if not missing(value) then do;
		if missing(first) then first = value;
		last = value;
	end;
